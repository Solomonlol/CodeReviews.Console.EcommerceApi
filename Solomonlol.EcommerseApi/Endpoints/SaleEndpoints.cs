using Microsoft.AspNetCore.Authorization;
using Solomonlol.EcommerseApi.Interfaces;
using Solomonlol.EcommerseApi.Models.Dto.Sale;
using Solomonlol.EcommerseApi.Services.Extensions.Filters;
using Solomonlol.EcommerseApi.Services.Extensions.Sort;
using System.Security.Claims;

namespace Solomonlol.EcommerseApi.Endpoints
{
    public static class SaleEndpoints
    {
        public static void MapSaleEndpoints(this WebApplication app)
        {
            //get one
            app.MapGet("api/v1/sales/{saleId}", [Authorize] async (int saleId, ISaleService service, CancellationToken ct) =>
            {
                var result = await service.Get(saleId, ct);
                return result.IsSuccess ? Results.Ok(result.Value) : Results.NotFound(result.Error);
            });
            //get all by user
            app.MapGet("api/v1/me/sales", [Authorize] async ([AsParameters] SaleFilter filter, [AsParameters] SortParams sortParams, ClaimsPrincipal user, ISaleService service, CancellationToken ct, int page = 1, int pageSize = 5) =>
            {
                var login = user.FindFirst(ClaimTypes.Name)?.Value;
                if (login == null) return Results.BadRequest();

                page = Math.Max(page, 1);
                pageSize = Math.Clamp(pageSize, 1, 30);
                var result = await service.GetAllByLogin(filter, sortParams, login, page, pageSize, ct);
                return result.IsSuccess ? Results.Ok(result.Value) : Results.NotFound(result.Error);
            });
            //get all
            app.MapGet("api/v1/sales", [Authorize(Roles = "Admin, Manager")] async ([AsParameters] SaleFilter filter, [AsParameters] SortParams sortParams, ISaleService service, CancellationToken ct, int page = 1, int pageSize = 5) =>
            {
                page = Math.Max(page, 1);
                pageSize = Math.Clamp(pageSize, 1, 30);
                var result = await service.GetAll(filter, sortParams, page, pageSize, ct);
                return result.IsSuccess ? Results.Ok(result.Value) : Results.Problem(result.Error);
            });
            //delete
            app.MapDelete("api/v1/sales/{saleId}", [Authorize(Roles = "Admin, Manager")] async (int saleId, ISaleService service, CancellationToken ct) =>
            {
                var result = await service.CloseSale(saleId, ct);
                return result.IsSuccess ? Results.NoContent() : Results.NotFound(result.Error);
            });
            //create
            app.MapPost("api/v1/sales", [Authorize] async (SaleDtoRequest item, ISaleService service, CancellationToken ct) =>
            {
                var result = await service.Create(item, ct);
                return result.IsSuccess ? Results.Created($"api/v1/sales/{result?.Value?.Id}", result?.Value) : Results.Conflict(result.Error);
            });
        }
    }
}
